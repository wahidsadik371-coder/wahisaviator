...
export function StrategyPanel() {
  ...
  return (
    <div className="space-y-3">
        ...
        <div className="rounded-xl border border-cyan-400/30 bg-cyan-400/5 p-3">
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="mt-0.5 text-[10px] text-white/50">{active.description}</div>
            </div>
            <button
              onClick={clearStrategy}
              className="tap-target rounded-lg bg-rose-500/20 px-2.5 py-1 text-[10px] font-bold text-rose-300 hover:bg-rose-500/30"
            >
              Stop
            </button>
          </div>
        ...
        {STRATEGY_PRESETS.map((s) => (
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <span className="text-[9px] uppercase text-white/40">{s.type}</span>
            </div>
            <div className="mt-0.5 text-[10px] text-white/50">{s.description}</div>
      ...
      <div className="rounded-xl border border-amber-400/20 bg-amber-400/5 p-2.5 text-[10px] text-amber-200/80">
        <Icon name="sparkles" className="mr-1 inline h-3 w-3" />
        Strategies auto-adjust your bet based on win/loss outcomes. Use responsibly — no strategy beats the house edge in the long run.
      </div>
    </div>
  );
}
